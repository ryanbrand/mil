...
class DataGenerator(object):
    def __init__(self, config={}):
        # Hyperparameters
        # the k in k-shot, defaults to 1
        self.update_batch_size = FLAGS.update_batch_size
        # number of demos used for test time
        ...
        # time horizon of the demo videos
        self.T = FLAGS.T
        # directory where the demo gifs are
        ...
        # path to the directory where demo files with states and actions are stored
        demo_file = FLAGS.demo_file
        # get's all the of the state, action files and sorts them
        demo_file = natsorted(glob.glob(demo_file + '/*pkl'))
        self.dataset_size = len(demo_file)
        # training_set_size=-1 for all but validation
        # 1500 for sim_reach, 693 for sim_push
    ...
    # sets: self.all_training_filenames: [list-of-filenames-for-some-task] (self.update_batch_size+self.test_batch_size) of length TOTAL_ITERS * META_BATCH_SIZE
    # sets: self.all_val_filenames: [list-of-gif-trial-filenames-for-some-task] of length TOTAL_ITERS / TEST_PRINT_INTERVAL * META_BATCH_SIZE
    # sets: self.training_batch_idx: maps train-iter -> batch_dict[task-idx] = list[path to trials for task]
    # sets: self.val_batch_idx: maps train-iter -> batch_dict[task-idx] = list[path to trials for task]
    def generate_batches(self, noisy=False):
        with Timer('Generating batches for each iteration'):
            if FLAGS.training_set_size != -1:
                ...
                offset = 0
            # get the gif-demo-dir path for each train_idx = [idx of pickle file used for training]
            # note that each gif dir has 24 gifs in it
            ...
            # do the same with the noisy data
            if noisy:
            ...
            # number of training iterations for the entire algo
            TOTAL_ITERS = FLAGS.metatrain_iterations
            self.all_training_filenames = []
            self.all_val_filenames = []
            # map each iteration to a dictionary that remembers the order entries were added
            self.training_batch_idx = {i: OrderedDict() for i in xrange(TOTAL_ITERS)}
            # only need val data to cover every eval that comes after each TEST_PRINT_INTERVAL
            ...
            for itr in xrange(TOTAL_ITERS):
                # randomly pick meta_batch_size tasks out of training tasks
                sampled_train_idx = random.sample(self.train_idx, self.meta_batch_size)
                # for each task sampled for training batch
                for idx in sampled_train_idx:
                    # get the gif-dir for that task
                    sampled_folder = train_img_folders[idx]
                    # get paths of all gifs in that folder
                    image_paths = natsorted(os.listdir(sampled_folder))
                    if FLAGS.experiment == 'sim_push':
                        # get 12 demonstrations for each task for sim push
                    ...
                    if not noisy:
                        # from all trials select update_batch_size + test_batch_size examples
                        # unsure why we are taking update_batch_size + test_batch_size samples maybe explained by main
                    ...
                    # add the path to each chosen gif-trial for the chosen task
                    self.all_training_filenames.extend(sampled_images)
                    # the dictionary for training iteration [itr] at key [task-idx] = idxs of trials chosen
                    self.training_batch_idx[itr][idx] = sampled_image_idx
                    if noisy:
                        self.noisy_training_batch_idx[itr][idx] = noisy_sampled_image_idx
                # we have gone through another test interval
                if itr != 0 and itr % TEST_PRINT_INTERVAL == 0:
                    # sample the validation tasks
                    sampled_val_idx = random.sample(self.val_idx, self.meta_batch_size)
                    # for each sampled validation task
                    for idx in sampled_val_idx:
                        # get path to gifs for selected tas
                        sampled_folder = val_img_folders[idx]
                        # sort all trials for task
                        image_paths = natsorted(os.listdir(sampled_folder))
                        if FLAGS.experiment == 'sim_push':
                            image_paths = image_paths[6:-6]
                        assert len(image_paths) == self.demos[idx]['demoX'].shape[0]
                        if noisy:
                            ...
                            assert len(noisy_image_paths) == self.noisy_demos[idx]['demoX'].shape[0]
                        if not noisy:
                            # sample update_batch_size+test_batch_size trials for task task-idx
                            sampled_image_idx = np.random.choice(range(len(image_paths)), size=self.update_batch_size+self.test_batch_size, replace=False) # True
                            sampled_images = [os.path.join(sampled_folder, image_paths[i]) for i in sampled_image_idx]
                        else:
                            noisy_sampled_image_idx = np.random.choice(range(len(noisy_image_paths)), size=self.update_batch_size, replace=False) # True
                        ...
                        # set val_batch[itr][task-idx] = gif file indexs for task-idx
    ...
    def make_batch_tensor(self, network_config, restore_iter=0, train=True):
        TEST_INTERVAL = 500
        # update_batch_size (train-shot) + test_batch_size (test-shot) * meta_batch_size (num-tasks / classes)
        ...
        if train:
            ...
            if restore_iter > 0:
                # get data to start from restore iter on
        ...
        # each gif has the following shape: T x H x W x C
        ...
        images = tf.train.batch(
                [image],
                batch_size = batch_image_size,  # update_batch_size (train-shot) + test_batch_size (test-shot) * meta_batch_size (num-tasks / classes)
        ...
        for i in xrange(self.meta_batch_size):
            # get train and test examples for task-idx task
        ...
        # dims: num-tasks X update_batch_size + test_batch_size X T X H X W X C
        return tf.stack(all_images)
        
    def generate_data_batch(self, itr, train=True):
        if train:
            # copy the state, action dict for each task in train_idx
            demos = {key: self.demos[key].copy() for key in self.train_idx}
            # idxs = batch_dict[task-idx] = list[path to trials for task]
        ...
        if not FLAGS.use_noisy_demos:
            # loop through task-idx, trial-idx, get action U, demos[k][demoU][v] = task k, trial v reshaped to be
            # test_batch_size + update_batch_size * T, -1
            # demoU: (24, 100, 7) = so you're taking all timesteps for test_batch_size+update_batch_size examples some how?
            # U: Meta-batch-size x (test_batch_size+update_batch_size)*self.T x action-dim
            # maybe v is a list of indexes of the trials so we get all the trials and then reshape that
